...
import { Alert, Image, Linking, Platform, Pressable, Share, StyleSheet, Switch, Text, TextInput, View } from 'react-native';
import { AlertTriangle, Bell, BookOpen, Camera, Check, CheckCircle2, Clock3, Copy, FileText, Flag, Image as ImageIcon, Link2, LogOut, MessageSquareWarning, Pause, PenLine, Play, Plus, RotateCcw, ShieldCheck, Trash2, UserMinus, Users } from 'lucide-react-native';
...
import { GOAL_PRESETS, goalLabel, goalPreset, isTimeGoal, sessionProgressTitle, sessionQuickStart } from '../data/goals';
...
export function AuthScreen({ model, navigate }: Omit<NavProps, 'back'>) {
  const [nickname, setNickname] = useState(model.pendingNickname ?? ''); const [busy, setBusy] = useState(false);
  useEffect(() => { if (model.pendingNickname != null) setNickname(model.pendingNickname); }, [model.pendingNickname]);
  const enter = () => { model.signIn(model.data.nickname); navigate('home'); };
  ...
  if (model.needsNicknameSetup) return <ScreenContainer bottomInset={32}><View style={styles.auth}><Brand /><View style={styles.authCopy}><StatusBadge label="Google 가입 완료" tone="success" /><Text style={styles.display}>앱에서 사용할{`\n`}닉네임을 정해 주세요.</Text><Text style={styles.body}>Google 계정의 표시 이름을 기본값으로 가져왔어요. 원하면 지금 바꿀 수 있습니다.</Text></View><Field label="닉네임" value={nickname} onChangeText={setNickname} placeholder="2~20자" /><PrimaryAction label="닉네임 저장하고 시작" icon={Check} loading={busy} disabled={nickname.trim().length < 2 || nickname.trim().length > 20} onPress={finishProfile} /></View></ScreenContainer>;
  return <ScreenContainer bottomInset={32}><View style={styles.auth}><View style={styles.authCopy}><Image source={require('../../assets/logo_4better.png')} accessibilityLabel="4better" resizeMode="contain" style={styles.authLogo} /><Text style={styles.authTagline}>친구 2~4명의 비공개 목표 리그</Text></View><View style={styles.stack}><PrimaryAction label="Google로 계속" icon={ShieldCheck} loading={busy} onPress={social} /><PrimaryAction label="미리보기 계정으로 시작" icon={Play} tone="quiet" onPress={enter} /></View><Text style={styles.legal}>계속하면 이용약관과 개인정보 처리방침에 동의하게 됩니다.</Text></View></ScreenContainer>;
}

export function CreateLeagueScreen({ model, back, navigate }: NavProps) {
  const [name, setName] = useState('퇴근 후 공부 클럽');
  ...
  const create = async () => {
    if (!model.authUserId) {
      Alert.alert('Google로 로그인해 주세요', '미리보기에서는 저장되지 않아요. 로그인하면 리그와 체크가 Supabase에 남습니다.');
      return;
    }
    setBusy(true);
    try {
      const league = await model.createLeague({
        name: name.trim(),
        kind,
        weeklyTarget: createTargetValue(kind, Number(target)),
        unit: preset.unit,
        plannedWeeks: weeks,
      });
  ...
  return (
    <ScreenContainer>
      ...
      <SectionHeader title="무엇을 함께할까요?" detail="목표는 앱에서 제공하는 프리셋만 고를 수 있어요" />
      <View style={styles.segment}>
        {GOAL_PRESETS.map((item) => (
          <Choice key={item.id} active={kind === item.id} label={item.label} onPress={() => setKind(item.id)} />
        ))}
      </View>
      <View style={styles.form}>
        <Field label="리그 이름" value={name} onChangeText={setName} />
        <Field label={isTimeGoal(kind) ? '주간 목표 시간' : '주간 목표 횟수'} value={target} onChangeText={setTarget} keyboardType="number-pad" suffix={isTimeGoal(kind) ? '시간' : '회'} />
        <Stepper label="진행 기간" value={weeks} unit="주" min={1} max={12} onChange={setWeeks} />
        <ReadOnlyRow label="주간 페널티" value="10,000원" />
        <ReadOnlyRow label="플랫폼 계산 수수료" value="총 페널티의 1%" />
        <ReadOnlyRow label="순위별 예상 분배" value="45 · 30 · 20 · 5%" />
      </View>
      ...
      <View style={styles.bottomAction}><PrimaryAction label="리그 만들고 체크 시작" loading={busy} onPress={create} disabled={!name.trim() || !Number(target) || busy} /></View>
    </ScreenContainer>
  );
}
...
export function SubmitScreen({ model, back, navigate }: NavProps) {
  ...
  const submit = async () => {
    ...
    try {
      if (remote && league) {
        if (isTimeGoal(league.kind) && (model.data.timerRunning || model.data.timerStartedAt || model.data.timerAccumulatedSeconds > 0)) {
          await model.finishTimer(note);
        } else {
          await model.submitCheck(isTimeGoal(league.kind) ? Math.max(1, Math.floor(model.timerSeconds / 60)) : 1, note);
        }
        navigate('home');
        return;
      }
...
function AdminMetric({ label, value }: { label: string; value: string }) { return <View style={styles.adminMetric}><Text style={styles.metricValue}>{value}</Text><Text style={styles.caption}>{label}</Text></View>; }

const styles = StyleSheet.create({
  flex: { flex: 1 }, stack: { gap: space.x2 }, section: { marginTop: space.x8 }, bottomAction: { marginTop: space.x6 }, twoActions: { gap: space.x2 }, textAction: { minHeight: 48, alignItems: 'center', justifyContent: 'center', marginTop: space.x3 }, link: { color: color.ink, ...type.label }, dangerLink: { color: color.danger, ...type.label },
  auth: { minHeight: 720, justifyContent: 'center', gap: space.x6, paddingVertical: space.x8 }, authCopy: { gap: space.x4, alignItems: 'center' }, authLogo: { width: 240, height: 88 }, authTagline: { color: color.inkMuted, ...type.label, textAlign: 'center' }, display: { color: color.ink, ...type.display }, title: { color: color.ink, ...type.title }, heading: { color: color.ink, ...type.heading }, body: { color: color.inkMuted, ...type.body }, label: { color: color.ink, ...type.label }, caption: { color: color.inkMuted, ...type.caption }, legal: { color: color.inkMuted, ...type.caption, textAlign: 'center' }, ruleHeading: { color: color.white, ...type.heading }, ruleCaption: { color: '#CBD5E1', ...type.caption },
  segment: { flexDirection: 'row', gap: space.x2, marginBottom: space.x5, flexWrap: 'wrap' }, choice: { minHeight: 48, paddingHorizontal: space.x4, borderRadius: radius.input, borderWidth: 1, borderColor: color.borderSubtle, backgroundColor: color.surface, flexDirection: 'row', alignItems: 'center', gap: space.x2 }, choiceActive: { backgroundColor: color.blue, borderColor: color.blue }, choiceText: { color: color.ink, ...type.label }, choiceTextActive: { color: color.white },
  form: { backgroundColor: color.surface, borderWidth: 1, borderColor: color.borderSubtle, borderRadius: radius.card, padding: space.x4, gap: space.x4, marginBottom: space.x5 }, field: { gap: space.x2 }, inputWrap: { minHeight: 52, borderWidth: 1, borderColor: color.borderSubtle, borderRadius: radius.input, flexDirection: 'row', alignItems: 'center', backgroundColor: '#FBFAF7' }, input: { flex: 1, minHeight: 50, paddingHorizontal: space.x4, color: color.ink, ...type.body }, suffix: { color: color.inkMuted, ...type.label, paddingRight: space.x4 }, multiline: { minHeight: 126, alignItems: 'flex-start' }, inputMultiline: { minHeight: 124, textAlignVertical: 'top', paddingTop: space.x3 },
  readRow: { minHeight: 44, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: space.x3, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: color.borderSubtle }, readLabel: { color: color.inkMuted, ...type.body, flex: 1 }, readValue: { color: color.ink, ...type.label, textAlign: 'right', flexShrink: 1 }, stepper: { flexDirection: 'row', alignItems: 'center', gap: space.x2 }, step: { width: 40, height: 40, borderRadius: radius.control, borderWidth: 1, borderColor: color.borderSubtle, alignItems: 'center', justifyContent: 'center' }, stepText: { color: color.ink, fontSize: 21 }, stepValue: { minWidth: 44, color: color.ink, ...type.label, textAlign: 'center' },
  ...
  metric: { backgroundColor: color.blueSoft, padding: space.x5, borderRadius: radius.card, marginBottom: space.x5 }, metricLabel: { color: color.inkMuted, ...type.caption }, metricValue: { color: color.ink, fontSize: 28, lineHeight: 36, fontWeight: '800' }, mediaRow: { minHeight: 72, borderTopWidth: 1, borderBottomWidth: 1, borderColor: color.borderSubtle, flexDirection: 'row', gap: space.x3, alignItems: 'center', marginVertical: space.x5 },
  timerPanel: { backgroundColor: color.ink, borderRadius: radius.league, paddingVertical: space.x10, paddingHorizontal: space.x5, alignItems: 'center', marginBottom: space.x5 }, timerRound: { color: color.aqua, ...type.label }, timer: { color: color.white, fontSize: 48, lineHeight: 62, fontWeight: '700', fontVariant: ['tabular-nums'], marginTop: space.x3 }, timerState: { color: '#CBD5E1', ...type.caption }, timerControls: { flexDirection: 'row', gap: space.x3, marginTop: space.x8 }, timerMain: { width: 60, height: 60, borderRadius: radius.round, backgroundColor: color.aqua, alignItems: 'center', justifyContent: 'center' }, timerFinish: { height: 60, borderRadius: radius.input, backgroundColor: color.orange, paddingHorizontal: space.x6, flexDirection: 'row', gap: space.x2, alignItems: 'center' }, timerFinishText: { color: color.white, ...type.label },
  ...
  scoreboard: { backgroundColor: color.ink, borderRadius: radius.league, padding: space.x5, marginBottom: space.x5 }, scoreboardLabel: { color: '#CBD5E1', ...type.caption }, scoreboardValue: { color: color.white, fontSize: 30, lineHeight: 39, fontWeight: '800' }, scoreboardMeta: { color: color.lime, ...type.caption, marginTop: space.x2 }, rank: { minHeight: 70, flexDirection: 'row', alignItems: 'center', gap: space.x3, borderBottomWidth: StyleSheet.hairlineWidth, borderColor: color.borderSubtle }, rankNo: { color: color.ink, fontSize: 18, fontWeight: '800', width: 20 }, smallAvatar: { width: 34, height: 34, borderRadius: radius.round, alignItems: 'center', justifyContent: 'center' }, initial: { color: color.ink, fontSize: 12, fontWeight: '800' }, reward: { color: color.blue, ...type.label },
