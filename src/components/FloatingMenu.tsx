...
import { Camera, ClipboardCheck, Home, Menu, Plus, Settings, X } from 'lucide-react-native';
...
export function FloatingMenu({ current, navigate, onNew }: { current: Route; navigate: (route: Route) => void; onNew: () => void }) {
  ...
  return (
    <View pointerEvents="box-none" style={[styles.host, { right }]}>
      {open ? <View style={styles.menu} accessibilityRole="menu">
        ...
        <Pressable accessibilityRole="menuitem" onPress={() => { setOpen(false); onNew(); }} style={({ pressed }) => [styles.item, pressed && styles.pressed]}><Plus size={18} color={color.primaryPressed} /><Text style={[styles.itemText, { color: color.primaryPressed }]}>새 리그</Text></Pressable>
      </View> : null}
...
const styles = StyleSheet.create({
  ...
  item: { minHeight: 44, borderRadius: radius.control, paddingHorizontal: space.x3, flexDirection: 'row', alignItems: 'center', gap: space.x3 }, selected: { backgroundColor: color.leagueDark },
  itemText: { color: color.ink, ...type.label }, selectedText: { color: color.white }, divider: { height: 1, backgroundColor: color.borderSubtle, marginVertical: space.x1 },
  trigger: { width: 54, height: 54, borderRadius: 18, backgroundColor: color.leagueDark, alignItems: 'center', justifyContent: 'center', ...elevation }, pressed: { transform: [{ scale: 0.98 }], opacity: 0.9 },
});
